...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(interior) and IsValid(capaldi) then
		...
		if tobool(GetConVarNumber("capaldi_advanced"))==true and (interior.flightmode==1 or interior.flightmode==2 or interior.flightmode==3) and interior.step==7 then
			interior:UpdateAdv(ply, true)
		else
	   	            capaldi:ToggleFlight()
