...
util.AddNetworkString("Player-Setcapaldi")
util.AddNetworkString("capaldi-SetHealth")
util.AddNetworkString("capaldi-Go")
util.AddNetworkString("capaldi-Stop")
util.AddNetworkString("capaldi-Explode")
util.AddNetworkString("capaldi-UnExplode")
util.AddNetworkString("capaldi-Flightmode")
util.AddNetworkString("capaldi-TakeDamage")
util.AddNetworkString("capaldi-FlightPhase")
util.AddNetworkString("capaldi-DoubleTrace")
util.AddNetworkString("capaldi-SpawnOffset")
util.AddNetworkString("capaldi-AdvancedMode")
util.AddNetworkString("capaldi-TeleportLock")
util.AddNetworkString("capaldi-LongFlight")
util.AddNetworkString("capaldi-PhysDamage")
util.AddNetworkString("capaldi-Phase")
util.AddNetworkString("capaldi-UpdateVis")
util.AddNetworkString("capaldi-SetInterior")
util.AddNetworkString("capaldi-SetViewmode")
util.AddNetworkString("capaldi-PlayerEnter")
util.AddNetworkString("capaldi-PlayerExit")
util.AddNetworkString("capaldi-SetLocked")
util.AddNetworkString("capaldi-SetRepairing")
util.AddNetworkString("capaldi-BeginRepair")
util.AddNetworkString("capaldi-FinishRepair")
util.AddNetworkString("capaldi-SetLight")
util.AddNetworkString("capaldi-SetStaticLight")
util.AddNetworkString("capaldi-SetPower")
util.AddNetworkString("capaldi-StopLong")
util.AddNetworkString("capaldi-Reappear")
util.AddNetworkString("capaldi-Materialize")
util.AddNetworkString("capaldi-SetVortex")
util.AddNetworkString("capaldi-NoCollideTeleport")

net.Receive("capaldi-TakeDamage", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_takedamage", net.ReadFloat())
	end
end)

net.Receive("capaldi-FlightPhase", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_flightphase", net.ReadFloat())
	end
end)

net.Receive("capaldi-DoubleTrace", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_doubletrace", net.ReadFloat())
	end
end)

net.Receive("capaldi-SpawnOffset", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_spawnoffset", net.ReadFloat())
	end
end)

net.Receive("capaldi-AdvancedMode", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_advanced", net.ReadFloat())
	end
end)

net.Receive("capaldi-TeleportLock", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_teleportlock", net.ReadFloat())
	end
end)

net.Receive("capaldi-NoCollideTeleport", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_nocollideteleport", net.ReadFloat())
	end
end)

net.Receive("capaldi-PhysDamage", function(len,ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		RunConsoleCommand("capaldi_physdamage", net.ReadFloat())
	end
end)

//make the player name invisible while capaldi is invisible
hook.Add("EV_ShowPlayerName", "capaldi-EV_ShowPlayerName", function(ply)
	if ply.capaldi and IsValid(ply.capaldi) and not ply.capaldi.visible then
		return false
	end
end)

//stop pewpew damaging when main damage is off
hook.Add("PewPew_ShouldDamage","capaldi-PewPew_ShouldDamage",function(_,ent,dmg)
	if ent and IsValid(ent) and ent:GetClass()=="sent_capaldi" then
...
function ENT:Initialize()
	self:SetModel( "models/doctorwho1200/capaldi/exteriorleg.mdl" )
	...
	if tobool(GetConVarNumber("capaldi_doubletrace"))==true then
		local trdata={}
	...
	local offset=0
	offset=GetConVarNumber("capaldi_spawnoffset")
	self.interior=ents.Create("sent_capaldi_interior")
	self.interior:SetPos(trace.HitPos+Vector(0,0,-600+offset))
	self.interior.capaldi=self
	...
	if IsValid(self.owner) then
		...
		self.extcol=Color(self.owner:GetInfoNum("capaldi_extcol_r",255),self.owner:GetInfoNum("capaldi_extcol_g",255),self.owner:GetInfoNum("capaldi_extcol_b",255))
...
function ENT:SetTrackingEnt(ent)
	if IsValid(ent) and not (ent==self) and not (ent==game.GetWorld()) and not (ent.capaldi_part) then
		self.tracking=true
...
function ENT:StartRepair()
	...
	self.repairing=true
	net.Start("capaldi-SetRepairing")
		net.WriteEntity(self)
		net.WriteBit(true)
...
function ENT:EndRepair()
	...
	if not self.repairwait then
		self:SetHP(100)
		net.Start("capaldi-FinishRepair")
			net.WriteEntity(self)
		net.Broadcast()
		self:FlashLight(1.5)
	end
...
function ENT:ShouldTakeDamage()
	return tobool(GetConVarNumber("capaldi_takedamage"))
end
...
function ENT:SetStaticLight(on)
	...
	net.Start("capaldi-SetStaticLight")
		net.WriteEntity(self)
...
function ENT:Go(vec,ang,nolongflight)
	if not self.moving and not self.repairing and self.power then
		...
		if tobool(GetConVarNumber("capaldi_nocollideteleport"))==true then
		...
		if self.attachedents then
			for k,v in pairs(self.attachedents) do
				if v.capaldi_part or v==self then
...
function ENT:Reappear()
	if self.vec then
		...
		for k,v in pairs(player.GetAll()) do
			if (self:GetPos():Distance(v:GetPos()) < 45) and not (v.capaldi and not v.capaldi_viewmode) then
...
function ENT:PlayerEnter( ply, override )
	if self.occupants then
		for k,v in pairs(self.occupants) do
			if ply==v and (not ply.capaldi_viewmode or ply.capaldi_skycamera) then return end
		end
	end
	if tobool(GetConVarNumber("capaldi_teleportlock"))==true and self.moving and not override then
		return
	end
	...
	if ply.capaldi and IsValid(ply.capaldi) then
		ply.capaldi:PlayerExit( ply )
	end
	ply.capaldi=self
	
	net.Start("Player-Setcapaldi")
		net.WriteEntity(ply)
		net.WriteEntity(self)
	net.Broadcast()
	ply.capaldi_viewmode=true
	net.Start("capaldi-SetViewmode")
		net.WriteBit(true)
	net.Send(ply)
	if self.interior and IsValid(self.interior) then
		ply:SetPos(self.interior:LocalToWorld(Vector(-270,0,89)))
...
function ENT:PlayerExit( ply, override )
	...
	net.Send(ply)
	ply.capaldi=nil
	ply.capaldi_viewmode=false
	ply.capaldiint_pos=nil
	ply.capaldiint_ang=nil
	net.Start("capaldi-SetViewmode")
		net.WriteBit(tobool(ply.capaldi_viewmode))
	net.Send(ply)
...
hook.Add("PlayerSpawn", "capaldi_PlayerSpawn", function( ply )
	local capaldi=ply.capaldi
	if capaldi and IsValid(capaldi) then
		if ply.capaldi_viewmode and capaldi.interior and IsValid(capaldi.interior) then
			ply:SetPos(capaldi.interior:LocalToWorld(Vector(-270,0,89)))
			local ang=capaldi.interior:GetAngles()+Angle(0,0,0)
			ply:SetEyeAngles(Angle(ang.p,ang.y,0))
		else
			capaldi:PlayerExit(ply)
		end
	end
end)

hook.Add("PlayerInitialSpawn", "capaldi_PlayerInitialSpawn", function( ply )
	timer.Simple(5,function()
		if not IsValid(ply) then return end
		for k,v in pairs(ents.FindByClass("sent_capaldi")) do
			if v and IsValid(v) then
				net.Start("capaldi-UpdateVis")
					net.WriteEntity(v)
...
function ENT:PhysicsUpdate( ph )
	...
	if self.flightmode and self.power then		
		...
		if self.tracking and not self.exploded then
			if IsValid(self.trackingent) then
				...
				if self.pilot and IsValid(self.pilot) and not self.pilot.capaldi_viewmode then
					local p=self.pilot
	...
	if self.occupants then
		for k,v in pairs(self.occupants) do
			if not v.capaldi_viewmode then
				v:SetPos(pos)
			end
		end
	end
...
function ENT:PhysicsCollide( data, physobj )
	if data.Speed and data.Speed>200 then
		...
		if tobool(GetConVarNumber("capaldi_physdamage"))==true then
...
function ENT:ToggleFlight()
	local flightphase=tobool(GetConVarNumber("capaldi_flightphase"))==true
...
function ENT:ToggleViewmode(ply,deldata)
	ply.capaldi_viewmode=(not ply.capaldi_viewmode) // true = inside, false = third-person view
	net.Start("capaldi-SetViewmode")
		net.WriteBit(tobool(ply.capaldi_viewmode))
	net.Send(ply)
	if ply.capaldi_viewmode then
		ply:UnSpectate()
		...
		if self.pilot and IsValid(self.pilot) and self.pilot==ply then
			...
			if not self.isomorphic and #tbl>0 then
				...
				if self.owner.capaldi==self and not self.owner.capaldi_viewmode then
					newpilot=self.owner
				end
		...
		if self.interior and IsValid(self.interior) then
			if not deldata and ply.capaldiint_pos and ply.capaldiint_ang then
				ply:SetPos(self.interior:LocalToWorld(ply.capaldiint_pos))
				ply:SetEyeAngles(ply.capaldiint_ang)
				ply.capaldiint_pos=nil
				ply.capaldiint_ang=nil
			else
				ply:SetPos(self.interior:GetPos()+Vector(320,360,-79))
				local ang=self.interior:GetAngles()+Angle(0,70,0)
				ply:SetEyeAngles(Angle(ang.p,ang.y,0))
			end
		end
	else
...
function ENT:Think()
	...
	if self.occupants then
		...
		for k,v in pairs(self.occupants) do
			...
			if CurTime()>self.viewmodecur and v:KeyDown(IN_USE) and not v.capaldi_viewmode then
	...
	if CurTime() > self.flightcur and self.pilot and IsValid(self.pilot) and self.pilot:KeyDown(IN_RELOAD) and not self.pilot.capaldi_viewmode and self.power then
		self:ToggleFlight()
	...
	if CurTime() > self.phasecur and self.pilot and IsValid(self.pilot) and self.pilot:KeyDown(IN_ATTACK2) and not self.pilot.capaldi_viewmode and self.power then
		self:TogglePhase()
	end
	...
	if not self.moving and not self.repairing and self.pilot and IsValid(self.pilot) and self.pilot:KeyDown(IN_ATTACK) and not self.pilot.capaldi_viewmode and self.power then
		if self.pilot.linked_capaldi and self.pilot.linked_capaldi==self and self.pilot.capaldi_vec and self.pilot.capaldi_ang then
			self:Go(self.pilot.capaldi_vec, self.pilot.capaldi_ang)
			self.pilot.capaldi_vec=nil
			self.pilot.capaldi_ang=nil
