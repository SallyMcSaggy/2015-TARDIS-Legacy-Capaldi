...
net.Receive("capaldiInt-Locations-GUI", function()
	...
	local function TransmitCoordinates()
		...
		if IsValid(interior) and IsValid(capaldi) and (not capaldi.moving or capaldi.invortex) and pos and ang then
			net.Start("capaldiInt-Locations-Send")
	...
	button.DoClick = function( button )
		if selected then
			selected.pos=capaldi:GetPos()
			selected.ang=capaldi:GetAngles()
			UpdateTextBoxes()
			changed=true
		else
			LocalPlayer():ChatPrint("No location selected.")
