...
function ENT:Initialize()
	self:SetModel( "models/doctorwho1200/capaldi/flightlever.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(interior) and IsValid(capaldi) then
		interior.usecur=CurTime()+1
		if tobool(GetConVarNumber("capaldi_advanced"))==true and (interior.flightmode==1 or interior.flightmode==2 or interior.flightmode==3) and interior.step==8 then
			interior:UpdateAdv(ply, true)
		else
			interior:UpdateAdv(ply, false)
			local success=capaldi:TogglePhysLock()
			if success then
				if capaldi.physlocked then
	...
	if IsValid(self.capaldi) then
		net.Start("capaldiInt-ControlSound")
			...
			net.WriteString("doctorwho1200/capaldi/flightlever.wav")
		net.Broadcast()
	end
end