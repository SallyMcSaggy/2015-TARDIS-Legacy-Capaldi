...
function ENT:Initialize()
	self:SetModel( "models/doctorwho1200/capaldi/hads.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(self.capaldi) then
		net.Start("capaldiInt-ControlSound")
			...
			net.WriteString("doctorwho1200/capaldi/switch.wav")
		net.Broadcast()
		
		local success=self.capaldi:ToggleHADS()
		if success then
			if self.capaldi.hads then
				ply:ChatPrint("Hostile Action Displacement System enabled.")
			else
				ply:ChatPrint("Hostile Action Displacement System disabled.")
			end
		end
	end
end