...
function ENT:Initialize()
	self:SetModel( "models/doctorwho1200/capaldi/isomorphic.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(interior) and IsValid(capaldi) then
		...
		local success=capaldi:IsomorphicToggle(ply)
		if success then
			if capaldi.isomorphic then
	...
	if IsValid(self.capaldi) then
		net.Start("capaldiInt-ControlSound")
			...
			net.WriteString("doctorwho1200/capaldi/isomorphic.wav")
		net.Broadcast()
	end
end