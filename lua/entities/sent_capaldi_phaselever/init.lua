...
function ENT:Initialize()
	self:SetModel( "models/doctorwho1200/capaldi/phaselever.mdl" )
...
function ENT:Toggle( bEnable, ply )
	...
	if IsValid(self.capaldi) then
		local success=self.capaldi:TogglePhase()
		if success then
			if self.capaldi.visible then
				ply:ChatPrint("TARDIS now visible.")
			elseif not self.capaldi.visible then
	...
	if IsValid(self.capaldi) then
		net.Start("capaldiInt-ControlSound")
			...
			net.WriteString("doctorwho1200/capaldi/phaselever.wav")
		net.Broadcast()
	end
end